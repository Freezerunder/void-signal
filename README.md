...
**Games open alongside the library, not on top of it** — a new tab in a browser, its own window in the desktop app, which has no tabs to open. In the app the launcher *is* the app, so navigating it away left nowhere to come back to; in a browser it simply lost your place. `electron/main.js` gives game windows the same options as the one they were opened from, and sends any non-bundled URL to the real browser rather than opening it chrome-less inside the app. If a popup blocker refuses the open, the card falls back to following the link so it never does nothing.

Tetris and 2048 turn their **← Library** link into "close this tab" whenever `window.opener` is set — true of a browser tab and an Electron window alike, and false when the page was opened straight from a bookmark, where there is nothing to close and the link should behave like a link. Void Signal has no back link, so there it is the tab close or ⌘W.
